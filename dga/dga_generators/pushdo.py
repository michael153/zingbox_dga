...
def generate_domains(num_per_dga, date):
    if not date:
        date = datetime.datetime.now()
    random.seed(date)
    domains = initDGA(num_per_dga, date, random.randint(1, 10000))
    index = 0
    ret = []
    for domain in domains:
        ret.append(domain[:8] + ".ru")
        index += 1
    return ret