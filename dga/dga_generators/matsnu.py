...
def choose_word(word_list, time=None):
    global domain_seed
    global next_domain
    global const1
    global const2
...
def generate_domain(time=None):
    ...
    while len(domain) < 24:
        if start_flag == 0:
            domain += choose_word(verbs, time)
            start_flag = 1
        domain += choose_word(nouns, time)
    domain += '.com'
    return domain

def generate_domains(num_per_dga, time):
   t = int(time.day)
   ret = []
   for i in range(0, num_per_dga):
      ret.append(generate_domain(t))
   return ret
